...
const seedFromAssets = async () => {
    try {
        const mongoUri = process.env.MONGODB_URI || process.env.MONGO_URI || "mongodb://127.0.0.1:27017/tomato";
        ...
        // Exact food_list from assets.js (MongoDB will auto-generate IDs)
        const food_list = [
        ...
        const result = await FoodModel.insertMany(food_list);
        console.log(`✅ Successfully inserted ${result.length} food items into MongoDB`);
